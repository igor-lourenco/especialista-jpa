...
public class _18_Usando_Expressao_Condicional_BETWEEN extends EntityManagerTest {


    @Test
    public void usandoExpressaoCondicional_BETWEEN_com_data(){ // Busca Pedido entre 04/02/2026 14:25:00 e 09/02/2026 18:45:00
...
//          + " WHERE p.dataCriacao BETWEEN :dataCriacaoInicial AND :dataCriacaoFinal"; // Mesma coisa que acima usando o >= e <=, porém os sinais são mais flexíveis
        ...
        criteriaQuery.where(criteriaBuilder.between(
            root.get(Pedido_.dataCriacao), dataCriacaoInicial, dataCriacaoFinal // WHERE p.dataCriacao BETWEEN :dataCriacaoInicial AND :dataCriacaoFinal
        ));
...
//            + " WHERE p.preco BETWEEN :precoInicial AND :precoFinal"
        ...
        criteriaQuery.where(criteriaBuilder.between(
            root.get(Produto_.preco), new BigDecimal("20.00"), new BigDecimal("499.00") //  WHERE p.preco BETWEEN :precoInicial AND :precoFinal
        ));
